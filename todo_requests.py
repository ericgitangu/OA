...
class TodoAPI:
    ...
    Problem Statement:
    Using the JSONPlaceholder API (https://jsonplaceholder.typicode.com/todos/),
    implement a class to interact with the API and demonstrate the functions
    of the requests package. The class should be able to fetch all todos,
    fetch a single todo by ID, create a new todo, update an existing todo,
    and delete a todo.
    ...
    create_todo(title, completed, user_id): Creates a new todo with the
    given title, completed status, and user ID.
    update_todo(todo_id, title=None, completed=None, user_id=None):
